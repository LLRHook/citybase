...
**State.** All UI state lives in the `CodebaseCity` component in `src/App.jsx`; a single `view` value switches `'city'` / `'kanban'` / `'analysis'`, and children receive props (no external store yet — Zustand is planned for Phase 2). The only persistent state is `workspaces.json` in Electron `userData` (`workspaceService`); there is no database.

**Fixture data.** `src/data/seed.js` is test-fixture data only — production renderer paths must never import it (the city is projected from real Git state; run history comes from `agentManager.listRuns()`). New test fixtures go in `seed.js`, never inline in components.
